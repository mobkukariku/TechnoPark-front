...
export const MembersList: FC = () => {
    const { membersForUsers, fetchingMembersForUsers, search, setSearch } = useMembersStore();
                ...
                {membersForUsers.map((item, index) => (
                    ...
                    >
                        <div className="flex flex-col justify-center gap-4 items-center text-center">
                            ...
                            <span className="text-sm text-gray-500">{item.memberProfile?.position}</span>
                            <Link href={`/members/${item.id}`}>
                                <p className="text-lg font-semibold text-blue-700 hover:underline">{item.name}</p>
                            </Link>
                        </div>
                    </motion.div>
                ))}
